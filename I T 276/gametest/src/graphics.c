...
void Init_Graphics()
{
    ...
    atexit(SDL_Quit);
        if(SDL_VideoModeOK(ScreenX, ScreenY, 32, SDL_FULLSCREEN | SDL_ANYFORMAT | SDL_HWSURFACE))
    {
        S_Data.xres = ScreenX;
        S_Data.yres = ScreenY;
        S_Data.depth = 32;
    ...
    else if(SDL_VideoModeOK(ScreenX, ScreenY, 16, SDL_FULLSCREEN | SDL_ANYFORMAT | SDL_HWSURFACE))
    {
        S_Data.xres = ScreenX;
        S_Data.yres = ScreenY;
        S_Data.depth = 16;
    ...
    else if(SDL_VideoModeOK(ScreenX, ScreenY, 16, SDL_FULLSCREEN | SDL_ANYFORMAT))
    {
        S_Data.xres = ScreenX;
        S_Data.yres = ScreenY;
        S_Data.depth = 16;
    ...
    if ( videobuffer == NULL )
    {
        fprintf(stderr, "Unable to set ScreenXxScreenY video: %s\n", SDL_GetError());
        exit(1);
    }
    ...
    temp = SDL_CreateRGBSurface(Vflags, (ScreenX*2), ScreenY, S_Data.depth,rmask, gmask,bmask,amask);
    if(temp == NULL)
	  {
